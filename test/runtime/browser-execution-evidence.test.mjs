...
import { runBrowserCommandAgent } from "../../runtime/electron/browser-command-agent.mjs";
import { createBrowserExecutionEvidence } from "../../runtime/electron/browser-execution-evidence.mjs";
  ...
  return {
    executeJavaScript: async (script = "") => {
      ...
      if (script.includes("matched: Boolean(element.isConnected && currentValue()")) {
        state.elements[0].previousDomRef = state.elements[0].domRef;
        state.elements[0].domRef = "snapshot-2";
...
test("the command runner refuses a bare model done but verifies a real fill-only run", async () => {
  ...
  assert.equal(filled.status, "success", JSON.stringify(filled));
