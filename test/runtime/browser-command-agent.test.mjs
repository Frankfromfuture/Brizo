...
} from "../../runtime/electron/browser-command-agent.mjs";
import {
  buildCtripFlightUrl,
...
} from "../../runtime/electron/ctrip-flight-flow.mjs";
import {
...
} from "../../runtime/electron/taobao-price-flow.mjs";
import { stripTrailingLinkPunctuation } from "../../runtime/shared/browser-input.mjs";
...
function mockWebContents(snapshot = { elements: [], pageText: "", title: "Test", url: "https://example.com/" }) {
  ...
  return {
    ...
    executeJavaScript: async (script = "") => {
      if (script.includes("const loginPattern") || script.includes("const auth = /login")) return null;
      ...
      if (script.includes("matched: Boolean(element.isConnected && currentValue()")) {
        ...
        const alreadySatisfied = snapshot.elements?.[0]?.value === value;
        ...
        return { alreadySatisfied, filled: true, matched: true };
      }
...
test("does not refill a control whose current value already matches", async () => {
  const webContents = mockWebContents({
    elements: [{ name: "名称", ref: "@e1", value: "Brizo" }],
    pageText: "",
    title: "Test",
