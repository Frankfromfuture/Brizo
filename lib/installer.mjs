...
async function managedFileStatus(filePath) {
  try {
    ...
    const hasVersionedManagedSignature = lines.some(line => (
      /^# installed-by-brizo-npm version=\S+ protocol=\d+$/u.test(line)
      || /^<!-- installed-by-brizo-npm version=\S+ protocol=\d+ -->$/u.test(line)
    ));
    if (
      hasVersionedManagedSignature
...
async function prepareBundle(paths, packageRoot) {
  ...
  await cp(path.join(packageRoot, "package.json"), path.join(paths.bundle, "package.json"));
