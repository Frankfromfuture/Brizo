...
小红书、豆瓣、B 站和微博的内容查询从网站公开首页进入，使用站内可见搜索框和筛选控件。到达结果页后核对搜索词、用户要求的最新或热门排序，以及每条结果的真实链接；标题、作者、日期、指标和摘要只能来自不同的可见结果卡片。结果少于用户要求时返回实际数量，登录或安全验证则交还用户处理。不得把只读搜索擅自扩大为点赞、收藏、关注、评论或发布。
