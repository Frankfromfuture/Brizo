...
function findControlAfterAction(action, beforeSnapshot, afterSnapshot) {
  ...
  // @eN is local to each observation: suggestions can reorder every control.
  // Follow the actual node across the two snapshots, never a coincidental
  // index or another identically labelled field with the requested value.
  if (before.domRef) {
    const matches = (afterSnapshot?.elements || []).filter((candidate) =>
      candidate.previousDomRef === before.domRef && stableControlMatch(before, candidate)
    );
    return matches.length === 1 ? matches[0] : null;
  }

  if (!before.fieldName) return null;
