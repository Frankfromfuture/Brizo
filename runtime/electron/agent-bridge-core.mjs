...
export async function startAgentBridgeCore({
  createSandbox,
  directory,
  host,
  ...
  if (typeof createSandbox !== "function") {
    throw new TypeError("startAgentBridgeCore requires createSandbox");
  }
  ...
  const socketPath = process.platform === "win32"
    ? `\\\\.\\pipe\\brizo-${nonce}`
    : path.join(directory, `bridge-${nonce}.sock`);
  ...
  const dispatch = async (request, signal) => {
    if (stopping || !sameSecret(request?.token, token)) {
      throw Object.assign(new Error("本机连接未获授权。"), { code: "UNAUTHORIZED" });
    }
    if (!isBridgeMethod(request?.method)) {
      throw Object.assign(new Error("不支持的桥接命令。"), { code: "METHOD_UNKNOWN" });
    }
    if (request.method === "ping") {
      return {
        protocol: BRIZO_PROTOCOL_VERSION,
        product: BRIZO_PRODUCT_NAME,
        pid: process.pid,
      };
    }
    if (request.method === "create") {
      if (sessions.size + creating >= 4) {
        throw Object.assign(new Error("最多同时打开四组 Agent 沙箱。"), { code: "SESSION_LIMIT" });
      }
      const goal = typeof request.args?.goal === "string"
        ? request.args.goal.trim().slice(0, 2000)
        : "";
      ...
      try {
        const sandbox = await createSandbox({
          id,
          goal,
          client: String(request.args?.client || "外部 Agent").slice(0, 80),
          url: request.args?.url,
          host,
          installNetworkPolicy,
          validateTarget,
          onClose: closedId => sessions.delete(closedId),
        });
        if (signal.aborted || stopping) {
          sandbox.destroy();
          throw new Error("创建请求已取消。");
        }
    ...
    if (!item || !sameSecret(request.capability, item.capability)) {
      throw Object.assign(new Error("沙箱不存在或不属于此连接。"), { code: "SESSION_SCOPE" });
    }
    ...
    socket.on("data", chunk => {
      ...
      void (async () => {
        ...
        try {
          response = {
            ok: true,
            result: await dispatch(JSON.parse(input.slice(0, end)), controller.signal),
          };
        } catch (error) {
          response = {
            ok: false,
            error: {
              code: error.code || "BRIZO_ERROR",
              message: error.message || "浏览器操作失败。",
            },
          };
        }
        answered = true;
