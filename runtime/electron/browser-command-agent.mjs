...
export async function snapshotBrowserPage(webContents) {
  ...
  return await evaluateBrowserPage(webContents, `
    (() => {
      ...
      const previousDomRefs = new WeakMap();
      ...
      const appendElement = (element, context, frameRef, frameDepth) => {
        ...
        elements.push({
          ...
          domRef,
          previousDomRef: previousDomRefs.get(element) || "",
          tag,
      ...
      const collectRoot = (root, context, frameRef = "", frameDepth = 0, depth = 0) => {
        ...
        seenRoots.add(root);
        root.querySelectorAll?.("[data-brizo-agent-ref]").forEach((element) => {
          previousDomRefs.set(element, element.getAttribute("data-brizo-agent-ref"));
          element.removeAttribute("data-brizo-agent-ref");
        });
...
export async function executeBrowserCommandAction({ action, command, sendInputEvents, signal, snapshot, validateNavigation, webContents }) {
  ...
  if (kind === "fill") {
    ...
    const filled = await evaluateBrowserPage(webContents, `
      (() => {
        ...
        if (currentValue() === ${JSON.stringify(value)}) {
          return { filled: true, matched: true, alreadySatisfied: true };
        }
        element.focus();
    ...
    return {
      status: "continued",
      result: filled.alreadySatisfied ? "already-satisfied" : "filled",
  ...
  if (kind === "select") {
    ...
    const selected = await evaluateBrowserPage(webContents, `
      (() => {
        ...
        if (String(element.value) === ${JSON.stringify(value)}) {
          return { selected: true, matched: true, alreadySatisfied: true };
        }
    ...
    return {
      status: "continued",
      result: selected.alreadySatisfied ? "already-satisfied" : "selected",
      postcondition: { kind: "control-state", verified: true },
    };
  }
  if (kind === "press") {
